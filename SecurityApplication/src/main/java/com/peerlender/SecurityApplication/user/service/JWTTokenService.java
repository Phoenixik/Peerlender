...
public class JWTTokenService implements TokenService, Clock {
    ...
    public Map<String, String> verify(String token) {
      final JwtParser parser = getParser().setSigningKey(secretKey);
    ...
    private String newToken(final Map<String, String> attributes, final int expiresInSec) {
        ...
        if(expiresInSec > 0) {
            ...
            claim.setExpiration(Date.from(expiresAt.toInstant(ZoneOffset.UTC)));
        } else{
    ...
    private static Map<String, String> parseClaims(final Supplier<Claims> toClaims) {
        try {
            ...
            claims.entrySet()
                    .stream().forEach(e -> builder.put(e.getKey(), String.valueOf(e.getValue())));
